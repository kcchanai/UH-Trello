...
const files = [
  ...
  'src/main.js', 'src/config.js', 'src/cloud-sync-controller.js', 'src/activity-ui.js',
...
const limits = {
  ...
  'src/main.js': 8_000, 'src/config.js': 4_000, 'src/cloud-sync-controller.js': 5_000, 'src/activity-ui.js': 5_000,
  ...
  'src/adapters/firebase-cloud-workspace.js': 22_000, 'src/granular-workspace.js': 6_000
};
let total = 0;
...
if (total > 180_000) throw new Error(`Initial source assets are ${total} bytes; budget is 180000.`);
const html = await readFile('index.html', 'utf8');
if (!html.includes('/src/main.js')) throw new Error('Vite module application entry is not loaded.');
console.log(`Performance budgets passed: ${total} bytes across ${files.length} source assets (budget 180000).`);
