export function initializeMembersUI(adapter) {
  ...
  const confirmAction = ({title, detail, label}) => new Promise(resolve => {
    const prompt = document.createElement('dialog'), card = document.createElement('div'), heading = document.createElement('h2'), text = document.createElement('p'), actions = document.createElement('div'), cancel = document.createElement('button'), confirm = document.createElement('button');
    ...
    const finish = result => { prompt.close(); prompt.remove(); resolve(result); }; cancel.addEventListener('click', () => finish(false)); confirm.addEventListener('click', () => finish(true)); prompt.addEventListener('cancel', event => { event.preventDefault(); finish(false); }); prompt.showModal(); cancel.focus();
  });
  ...
  const refresh = async () => {
    ...
    try {
      ...
      members.forEach(member => {
        ...
        if (owner && member.role !== 'owner') { const select = document.createElement('select'); ['editor','viewer'].forEach(value => { const option = new Option(value, value, false, value === member.role); select.add(option); }); select.addEventListener('change', async () => { const next=select.value; if (!await confirmAction({title:'Change member role?', detail:`Change this member from ${member.role} to ${next}.`, label:'Change role'})) { select.value=member.role; return; } await adapter.changeMemberRole(workspaceId, member.uid, next); refresh(); }); const remove = document.createElement('button'); remove.type = 'button'; remove.className = 'button button-quiet'; remove.textContent = 'Remove'; remove.addEventListener('click', async () => { if (!await confirmAction({title:'Remove member?', detail:'They will immediately lose this cloud workspace access. Local browser data is unaffected.', label:'Remove member'})) return; await adapter.removeMember(workspaceId, member.uid); refresh(); }); row.append(select, remove); }
        if (!owner && member.uid === session.uid && member.role !== 'owner') { const leave = document.createElement('button'); leave.type = 'button'; leave.className = 'button button-quiet'; leave.textContent = 'Leave workspace'; leave.addEventListener('click', async () => { if (!await confirmAction({title:'Leave workspace?', detail:'You will lose cloud workspace access. This browser’s local workspace is unaffected.', label:'Leave workspace'})) return; await adapter.leaveWorkspace(workspaceId); dialog.close(); open.hidden = true; }); row.append(leave); }
  ...
  transferForm.addEventListener('submit', async event => { event.preventDefault(); if (!owner || !workspaceId || !successor.value) return; if (!await confirmAction({title:'Transfer ownership?', detail:'The selected member becomes owner and your role will change. This cannot be undone without the new owner.', label:'Transfer ownership'})) return; const submit = transferForm.querySelector('button[type="submit"]'); submit.disabled = true; message('Transferring ownership…'); try { await adapter.transferOwnership({workspaceId, successorUid:successor.value, formerOwnerRole:formerOwnerRole.value}); message('Ownership transferred. Your local workspace remains unchanged.'); await refresh(); } catch (error) { console.error('Flowboard ownership transfer failed.', error); message('Ownership transfer was not completed. No local data changed.'); } finally { submit.disabled = false; } });
